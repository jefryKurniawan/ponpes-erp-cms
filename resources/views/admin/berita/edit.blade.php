...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h2 class="font-headline-md text-headline-md text-on-surface mb-2">Edit Berita</h2>
        <p class="font-body-md text-body-md text-on-surface-variant">Perbarui informasi berita.</p>
    </div>
    ...
    <div class="cms-card bg-surface-container-lowest rounded-xl p-6 border border-outline-variant/20">
        <form action="{{ route('admin.berita.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="judul" class="block font-label-md text-on-surface mb-2">Judul</label>
                <input type="text" class="cms-input w-full @error('judul') border-error @enderror" id="judul" name="judul" value="{{ old('judul', $post->judul) }}" required placeholder="Contoh: Pesantren Al-Hikmah Gelar Kajian Ramadan 1447 H">
                @error('judul')
            ...
            <div>
                ...
                <input type="text" class="cms-input w-full @error('slug') border-error @enderror" id="slug" name="slug" value="{{ old('slug', $post->slug) }}" placeholder="Biarkan kosong untuk digenerate otomatis">
                @error('slug')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="datetime-local" class="cms-input w-full @error('published_at') border-error @enderror" id="published_at" name="published_at" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : '') }}">
                @error('published_at')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea class="cms-input w-full @error('isi') border-error @enderror" id="isi" name="isi" rows="10" placeholder="Tulis isi berita di sini...">{{ old('isi', $post->isi) }}</textarea>
                @error('isi')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @error('thumbnail')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-on-surface-variant text-sm mt-1">Format: JPG, PNG, maksimal 2MB. Biarkan kosong jika tidak ingin mengganti.</p>
            </div>

            <div class="flex gap-3 pt-4">
                <button type="submit" class="bg-primary text-on-primary px-6 py-2 rounded-lg font-label-md hover:bg-primary/90 transition-colors">Perbarui Berita</button>
