    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="cms-card bg-surface-container-lowest rounded-xl p-4 border border-outline-variant/20 mb-6">
            <form method="GET" action="{{ route('inventaris.index') }}" class="flex gap-3">
                ...
                @if(request('keyword'))
                    <a href="{{ route('inventaris.index') }}" class="border border-outline bg-surface text-on-surface px-6 py-2 rounded-lg font-label-sm hover:bg-surface-container transition-colors">Reset</a>
                @endif
            </form>
        </div>
